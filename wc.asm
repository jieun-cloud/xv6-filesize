...
putc(int fd, char c)
{
 ...
 4e6:	e8 fd fe ff ff       	call   3e8 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 5b8:	e8 2b fe ff ff       	call   3e8 <write>
...
putc(int fd, char c)
{
 ...
 5da:	e8 09 fe ff ff       	call   3e8 <write>
...
putc(int fd, char c)
{
 ...
 685:	e8 5e fd ff ff       	call   3e8 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
{
  write(fd, &c, 1);
 6f4:	e8 ef fc ff ff       	call   3e8 <write>
...
morecore(uint nu)
{
 ...
 82f:	e8 1c fc ff ff       	call   450 <sbrk>
  if(p == (char*)-1)
